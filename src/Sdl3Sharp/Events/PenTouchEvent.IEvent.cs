...
partial struct Event
{
	...
	partial interface IUnionMembers
	{
		...
		static abstract Event Create(in PenTouchEvent value);
		...
		/// </para>
		/// </remarks>
		bool TryGetValue(out PenTouchEvent value);
	}
#endif
	...
	static Event IUnionMembers.Create(in PenTouchEvent value) => From(in value);

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly bool IUnionMembers.TryGetValue(out PenTouchEvent value) => TryGet(out value);
#endif
}

partial struct PenTouchEvent : IEvent<PenTouchEvent>
{
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	private static bool AcceptsEventType(EventType type) => type is EventType.PenDown or EventType.PenUp;

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	static bool IEvent<PenTouchEvent>.TryReadFromEvent(ref readonly Event @event, out PenTouchEvent result)
	{
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly void IEvent<PenTouchEvent>.WriteToEvent(ref Event @event) => @event.PTouch = this;
}
