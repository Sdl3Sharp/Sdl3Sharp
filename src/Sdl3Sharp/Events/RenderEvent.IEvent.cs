...
partial struct Event
{
	...
	partial interface IUnionMembers
	{
		...
		static abstract Event Create(in RenderEvent value);
		...
		/// </para>
		/// </remarks>
		bool TryGetValue(out RenderEvent value);
	}
#endif
	...
	static Event IUnionMembers.Create(in RenderEvent value) => From(in value);

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly bool IUnionMembers.TryGetValue(out RenderEvent value) => TryGet(out value);
#endif
}

partial struct RenderEvent : IEvent<RenderEvent>
{
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
#pragma warning disable CS0618 // We can use them here, that's what they're there for
	private static bool AcceptsEventType(EventType type) => type is >= EventType.RenderFirst and <= EventType.RenderLast;
#pragma warning restore CS0618

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	static bool IEvent<RenderEvent>.TryReadFromEvent(ref readonly Event @event, out RenderEvent result)
	{
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly void IEvent<RenderEvent>.WriteToEvent(ref Event @event) => @event.Render = this;
}
