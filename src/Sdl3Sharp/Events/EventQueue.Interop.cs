...
partial class EventQueue
{
	...
	private unsafe static CBool EventFilterImpl(void* userdata, Event* @event)
	{
		...
		{
			return eventFilter(ref Unsafe.AsRef<Event>(@event));
		}
	...
	{
		if (userdata is not null && GCHandle.FromIntPtr(unchecked((IntPtr)userdata)) is { IsAllocated: true, Target: EventWatch eventWatch })
		{
			eventWatch(ref Unsafe.AsRef<Event>(@event));
		}
