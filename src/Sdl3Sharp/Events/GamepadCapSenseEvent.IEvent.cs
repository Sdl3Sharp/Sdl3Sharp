...
partial struct Event
{
	...
	partial interface IUnionMembers
	{
		...
		static abstract Event Create(in GamepadCapSenseEvent value);
		...
		/// </para>
		/// </remarks>
		bool TryGetValue(out GamepadCapSenseEvent value);
	}
#endif

	[FieldOffset(0)] internal GamepadCapSenseEvent GCapsense;

#if NET11_0_OR_GREATER
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	static Event IUnionMembers.Create(in GamepadCapSenseEvent value) => From(in value);

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly bool IUnionMembers.TryGetValue(out GamepadCapSenseEvent value) => TryGet(out value);
#endif
}

partial struct GamepadCapSenseEvent : IEvent<GamepadCapSenseEvent>
{
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	private static bool AcceptsEventType(EventType type) => type is EventType.GamepadCapSenseTouched or EventType.GamepadCapSenseReleased;

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	static bool IEvent<GamepadCapSenseEvent>.TryReadFromEvent(ref readonly Event @event, out GamepadCapSenseEvent result)
	{
		if (AcceptsEventType(@event.Type))
		{
			result = @event.GCapsense;
			return true;
		}
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly void IEvent<GamepadCapSenseEvent>.WriteToEvent(ref Event @event) => @event.GCapsense = this;
}
