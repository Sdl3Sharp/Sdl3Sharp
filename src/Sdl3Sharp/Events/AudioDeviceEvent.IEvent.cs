...
partial struct Event
{
	...
	partial interface IUnionMembers
	{
		...
		static abstract Event Create(in AudioDeviceEvent value);
		...
		/// </para>
		/// </remarks>
		bool TryGetValue(out AudioDeviceEvent value);
	}
#endif
	...
	static Event IUnionMembers.Create(in AudioDeviceEvent value) => From(in value);

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly bool IUnionMembers.TryGetValue(out AudioDeviceEvent value) => TryGet(out value);
#endif
}

partial struct AudioDeviceEvent : IEvent<AudioDeviceEvent>
{
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
#pragma warning disable CS0618 // We can use them here, that's what they're there for
	private static bool AcceptsEventType(EventType type) => type is >= EventType.AudioDeviceFirst and <= EventType.AudioDeviceLast;
#pragma warning restore CS0618

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	static bool IEvent<AudioDeviceEvent>.TryReadFromEvent(ref readonly Event @event, out AudioDeviceEvent result)
	{
		if (AcceptsEventType(@event.Type))
		{
			result = @event.ADevice;
			return true;
		}
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly void IEvent<AudioDeviceEvent>.WriteToEvent(ref Event @event) => @event.ADevice = this;
}
