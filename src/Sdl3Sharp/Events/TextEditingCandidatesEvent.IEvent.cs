...
partial struct Event
{
	...
	partial interface IUnionMembers
	{
		...
		static abstract Event Create(in TextEditingCandidatesEvent value);
		...
		/// </para>
		/// </remarks>
		bool TryGetValue(out TextEditingCandidatesEvent value);
	}
#endif
	...
	static Event IUnionMembers.Create(in TextEditingCandidatesEvent value) => From(in value);

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly bool IUnionMembers.TryGetValue(out TextEditingCandidatesEvent value) => TryGet(out value);
#endif
}

partial struct TextEditingCandidatesEvent : IEvent<TextEditingCandidatesEvent>
{
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	private static bool AcceptsEventType(EventType type) => type is EventType.TextEditingCandidates;

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	static bool IEvent<TextEditingCandidatesEvent>.TryReadFromEvent(ref readonly Event @event, out TextEditingCandidatesEvent result)
	{
		if (AcceptsEventType(@event.Type))
		{
			result = @event.EditCandidates;
			return true;
		}
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly void IEvent<TextEditingCandidatesEvent>.WriteToEvent(ref Event @event) => @event.EditCandidates = this;
}
