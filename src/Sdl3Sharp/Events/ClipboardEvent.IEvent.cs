...
partial struct Event
{
	...
	partial interface IUnionMembers
	{
		...
		static abstract Event Create(in ClipboardEvent value);
		...
		/// </para>
		/// </remarks>
		bool TryGetValue(out ClipboardEvent value);
	}
#endif
	...
	static Event IUnionMembers.Create(in ClipboardEvent value) => From(in value);

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly bool IUnionMembers.TryGetValue(out ClipboardEvent value) => TryGet(out value);
#endif
}

partial struct ClipboardEvent : IEvent<ClipboardEvent>
{
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
#pragma warning disable CS0618 // We can use them here, that's what they're there for
	private static bool AcceptsEventType(EventType type) => type is >= EventType.ClipboardFirst and <= EventType.ClipboardLast;
#pragma warning restore CS0618

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	static bool IEvent<ClipboardEvent>.TryReadFromEvent(ref readonly Event @event, out ClipboardEvent result)
	{
		if (AcceptsEventType(@event.Type))
		{
			result = @event.Clipboard;
			return true;
		}
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly void IEvent<ClipboardEvent>.WriteToEvent(ref Event @event) => @event.Clipboard = this;
}
