...
partial struct Event
{
	...
	partial interface IUnionMembers
	{
		...
		static abstract Event Create(in GamepadTouchpadEvent value);
		...
		/// </para>
		/// </remarks>
		bool TryGetValue(out GamepadTouchpadEvent value);
	}
#endif
	...
	static Event IUnionMembers.Create(in GamepadTouchpadEvent value) => From(in value);

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly bool IUnionMembers.TryGetValue(out GamepadTouchpadEvent value) => TryGet(out value);
#endif
}

partial struct GamepadTouchpadEvent : IEvent<GamepadTouchpadEvent>
{
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	private static bool AcceptsEventType(EventType type) => type is >= EventType.GamepadTouchpadDown and <= EventType.GamepadTouchpadUp;

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	static bool IEvent<GamepadTouchpadEvent>.TryReadFromEvent(ref readonly Event @event, out GamepadTouchpadEvent result)
	{
		if (AcceptsEventType(@event.Type))
		{
			result = @event.GTouchpad;
			return true;
		}
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly void IEvent<GamepadTouchpadEvent>.WriteToEvent(ref Event @event) => @event.GTouchpad = this;
}
