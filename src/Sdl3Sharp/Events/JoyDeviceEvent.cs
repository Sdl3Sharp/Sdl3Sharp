...
/// <summary>
/// Represents an event that occurs when a <see cref="Input.Joystick"/> device is added or removed or when a <see cref="Input.Joystick"/> update is completed
/// </summary>
/// <remarks>
/// <para>
...
/// </para>
/// <para>
/// SDL will send <see cref="EventType.JoystickAdded"/> (<see cref="JoyDeviceEvent"/>) events for joystick devices that are already connected when <see cref="Sdl(Sdl3Sharp.Sdl.BuildAction?)">SDL is initialized</see>.
/// </para>
/// </remarks>
[DebuggerDisplay($"{{{nameof(DebuggerDisplay)},nq}}")]
[StructLayout(LayoutKind.Sequential)]
public partial struct JoyDeviceEvent : IFormattable, ISpanFormattable
{
	...
	/// When setting this property, the given <see cref="EventType"/> is not a valid type for a <see cref="JoyDeviceEvent"/>
	/// </exception>
	public required EventType Type
	{
		...
		set
		{
			...
			{
				[DoesNotReturn]
				static void failInvalidEventType(EventType type) => throw new ArgumentException($"Invalid event type for {nameof(JoyDeviceEvent)}: {type}.", nameof(value));
