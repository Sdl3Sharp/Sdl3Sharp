...
partial struct Event
{
	...
	partial interface IUnionMembers
	{
		...
		static abstract Event Create(in NotificationEvent value);
		...
		/// </para>
		/// </remarks>
		bool TryGetValue(out NotificationEvent value);
	}
#endif

	[FieldOffset(0)] internal NotificationEvent Notification;

#if NET11_0_OR_GREATER
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	static Event IUnionMembers.Create(in NotificationEvent value) => From(in value);

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly bool IUnionMembers.TryGetValue(out NotificationEvent value) => TryGet(out value);
#endif
}

partial struct NotificationEvent : IEvent<NotificationEvent>
{
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
#pragma warning disable CS0618 // We can use them here, that's what they're there for
	private static bool AcceptsEventType(EventType type) => type is >= EventType.NotificationFirst and <= EventType.NotificationLast;
#pragma warning restore CS0618

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	static bool IEvent<NotificationEvent>.TryReadFromEvent(ref readonly Event @event, out NotificationEvent result)
	{
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly void IEvent<NotificationEvent>.WriteToEvent(ref Event @event) => @event.Notification = this;
}
