...
partial struct Event
{
	...
	partial interface IUnionMembers
	{
		...
		static abstract Event Create(in PenProximityEvent value);
		...
		/// </para>
		/// </remarks>
		bool TryGetValue(out PenProximityEvent value);
	}
#endif
	...
	static Event IUnionMembers.Create(in PenProximityEvent value) => From(in value);

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly bool IUnionMembers.TryGetValue(out PenProximityEvent value) => TryGet(out value);
#endif
}

partial struct PenProximityEvent : IEvent<PenProximityEvent>
{
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	private static bool AcceptsEventType(EventType type) => type is EventType.PenProximityIn or EventType.PenProximityOut;

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	static bool IEvent<PenProximityEvent>.TryReadFromEvent(ref readonly Event @event, out PenProximityEvent result)
	{
		if (AcceptsEventType(@event.Type))
		{
			result = @event.PProximity;
			return true;
		}
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly void IEvent<PenProximityEvent>.WriteToEvent(ref Event @event) => @event.PProximity = this;
}
