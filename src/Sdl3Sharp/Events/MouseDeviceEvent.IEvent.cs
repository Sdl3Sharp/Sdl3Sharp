...
partial struct Event
{
	...
	partial interface IUnionMembers
	{
		...
		static abstract Event Create(in MouseDeviceEvent value);
		...
		/// </para>
		/// </remarks>
		bool TryGetValue(out MouseDeviceEvent value);
	}
#endif
	...
	static Event IUnionMembers.Create(in MouseDeviceEvent value) => From(in value);

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly bool IUnionMembers.TryGetValue(out MouseDeviceEvent value) => TryGet(out value);
#endif
}

partial struct MouseDeviceEvent : IEvent<MouseDeviceEvent>
{
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	private static bool AcceptsEventType(EventType type) => type is EventType.MouseAdded or EventType.MouseRemoved;

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	static bool IEvent<MouseDeviceEvent>.TryReadFromEvent(ref readonly Event @event, out MouseDeviceEvent result)
	{
		if (AcceptsEventType(@event.Type))
		{
			result = @event.MDevice;
			return true;
		}
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly void IEvent<MouseDeviceEvent>.WriteToEvent(ref Event @event) => @event.MDevice = this;
}
