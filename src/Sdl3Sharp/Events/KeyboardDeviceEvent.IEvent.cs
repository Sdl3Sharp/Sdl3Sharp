...
partial struct Event
{
	...
	partial interface IUnionMembers
	{
		...
		static abstract Event Create(in KeyboardDeviceEvent value);
		...
		/// </para>
		/// </remarks>
		bool TryGetValue(out KeyboardDeviceEvent value);
	}
#endif
	...
	static Event IUnionMembers.Create(in KeyboardDeviceEvent value) => From(in value);

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly bool IUnionMembers.TryGetValue(out KeyboardDeviceEvent value) => TryGet(out value);
#endif
}

partial struct KeyboardDeviceEvent : IEvent<KeyboardDeviceEvent>
{
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	private static bool AcceptsEventType(EventType type) => type is EventType.KeyboardAdded or EventType.KeyboardRemoved;

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	static bool IEvent<KeyboardDeviceEvent>.TryReadFromEvent(ref readonly Event @event, out KeyboardDeviceEvent result)
	{
		if (AcceptsEventType(@event.Type))
		{
			result = @event.KDevice;
			return true;
		}
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly void IEvent<KeyboardDeviceEvent>.WriteToEvent(ref Event @event) => @event.KDevice = this;
}
