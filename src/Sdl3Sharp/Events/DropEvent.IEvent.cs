...
partial struct Event
{
	...
	partial interface IUnionMembers
	{
		...
		static abstract Event Create(in DropEvent value);
		...
		/// </para>
		/// </remarks>
		bool TryGetValue(out DropEvent value);
	}
#endif
	...
	static Event IUnionMembers.Create(in DropEvent value) => From(in value);

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly bool IUnionMembers.TryGetValue(out DropEvent value) => TryGet(out value);
#endif
}

partial struct DropEvent : IEvent<DropEvent>
{
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
#pragma warning disable CS0618 // We can use them here, that's what they're there for
	private static bool AcceptsEventType(EventType type) => type is >= EventType.DropFirst and <= EventType.DropLast;
#pragma warning restore CS0618

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	static bool IEvent<DropEvent>.TryReadFromEvent(ref readonly Event @event, out DropEvent result)
	{
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly void IEvent<DropEvent>.WriteToEvent(ref Event @event) => @event.Drop = this;
}
