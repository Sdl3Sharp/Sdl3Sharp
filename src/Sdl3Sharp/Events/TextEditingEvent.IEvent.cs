...
partial struct Event
{
	...
	partial interface IUnionMembers
	{
		...
		static abstract Event Create(in TextEditingEvent value);
		...
		/// </para>
		/// </remarks>
		bool TryGetValue(out TextEditingEvent value);
	}
#endif
	...
	static Event IUnionMembers.Create(in TextEditingEvent value) => From(in value);

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly bool IUnionMembers.TryGetValue(out TextEditingEvent value) => TryGet(out value);
#endif
}

partial struct TextEditingEvent : IEvent<TextEditingEvent>
{
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	private static bool AcceptsEventType(EventType type) => type is EventType.TextEditing;

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	static bool IEvent<TextEditingEvent>.TryReadFromEvent(ref readonly Event @event, out TextEditingEvent result)
	{
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly void IEvent<TextEditingEvent>.WriteToEvent(ref Event @event) => @event.Edit = this;
}
