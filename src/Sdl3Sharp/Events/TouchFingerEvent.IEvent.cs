...
partial struct Event
{
	...
	partial interface IUnionMembers
	{
		...
		static abstract Event Create(in TouchFingerEvent value);
		...
		/// </para>
		/// </remarks>
		bool TryGetValue(out TouchFingerEvent value);
	}
#endif
	...
	static Event IUnionMembers.Create(in TouchFingerEvent value) => From(in value);

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly bool IUnionMembers.TryGetValue(out TouchFingerEvent value) => TryGet(out value);
#endif
}

partial struct TouchFingerEvent : IEvent<TouchFingerEvent>
{
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
#pragma warning disable CS0618 // We can use them here, that's what they're there for
	private static bool AcceptsEventType(EventType type) => type is >= EventType.FingerFirst and <= EventType.FingerLast;
#pragma warning restore CS0618

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	static bool IEvent<TouchFingerEvent>.TryReadFromEvent(ref readonly Event @event, out TouchFingerEvent result)
	{
		if (AcceptsEventType(@event.Type))
		{
			result = @event.TFinger;
			return true;
		}
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly void IEvent<TouchFingerEvent>.WriteToEvent(ref Event @event) => @event.TFinger = this;
}
