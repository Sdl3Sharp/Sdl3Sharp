...
partial struct Event
{
	...
	partial interface IUnionMembers
	{
		...
		static abstract Event Create(in UnmanagedUserEvent value);
		...
		/// </para>
		/// </remarks>
		bool TryGetValue(out UnmanagedUserEvent value);
	}
#endif
	...
	static Event IUnionMembers.Create(in UnmanagedUserEvent value) => From(in value);

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly bool IUnionMembers.TryGetValue(out UnmanagedUserEvent value) => TryGet(out value);
#endif
}

partial struct UnmanagedUserEvent : IEvent<UnmanagedUserEvent>
{
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
#pragma warning disable CS0618 // We can use them here, that's what they're there for
	private static bool AcceptsEventType(EventType type) => type is >= EventType.User and <= EventType.Last;
#pragma warning restore CS0618

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	static bool IEvent<UnmanagedUserEvent>.TryReadFromEvent(ref readonly Event @event, out UnmanagedUserEvent result)
	{
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly void IEvent<UnmanagedUserEvent>.WriteToEvent(ref Event @event) => @event.User = this;
}
