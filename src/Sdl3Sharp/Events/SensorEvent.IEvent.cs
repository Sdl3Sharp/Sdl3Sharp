...
partial struct Event
{
	...
	partial interface IUnionMembers
	{
		...
		static abstract Event Create(in SensorEvent value);
		...
		/// </para>
		/// </remarks>
		bool TryGetValue(out SensorEvent value);
	}
#endif
	...
	static Event IUnionMembers.Create(in SensorEvent value) => From(in value);

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly bool IUnionMembers.TryGetValue(out SensorEvent value) => TryGet(out value);
#endif
}

partial struct SensorEvent : IEvent<SensorEvent>
{
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
#pragma warning disable CS0618 // We can use them here, that's what they're there for
	private static bool AcceptsEventType(EventType type) => type is >= EventType.SensorFirst and <= EventType.SensorLast;
#pragma warning restore CS0618

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	static bool IEvent<SensorEvent>.TryReadFromEvent(ref readonly Event @event, out SensorEvent result)
	{
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly void IEvent<SensorEvent>.WriteToEvent(ref Event @event) => @event.Sensor = this;
}
