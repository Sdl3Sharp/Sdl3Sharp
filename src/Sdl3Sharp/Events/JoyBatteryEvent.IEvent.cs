...
partial struct Event
{
	...
	partial interface IUnionMembers
	{
		...
		static abstract Event Create(in JoyBatteryEvent value);
		...
		/// </para>
		/// </remarks>
		bool TryGetValue(out JoyBatteryEvent value);
	}
#endif
	...
	static Event IUnionMembers.Create(in JoyBatteryEvent value) => From(in value);

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly bool IUnionMembers.TryGetValue(out JoyBatteryEvent value) => TryGet(out value);
#endif
}

partial struct JoyBatteryEvent : IEvent<JoyBatteryEvent>
{
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	private static bool AcceptsEventType(EventType type) => type is EventType.JoystickBatteryUpdated;

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	static bool IEvent<JoyBatteryEvent>.TryReadFromEvent(ref readonly Event @event, out JoyBatteryEvent result)
	{
		if (AcceptsEventType(@event.Type))
		{
			result = @event.JBattery;
			return true;
		}
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly void IEvent<JoyBatteryEvent>.WriteToEvent(ref Event @event) => @event.JBattery = this;
}
