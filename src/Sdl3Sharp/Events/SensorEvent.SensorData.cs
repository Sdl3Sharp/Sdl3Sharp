...
partial struct SensorEvent
{
	/// <summary>
	/// Represents the sensor data of a <see cref="SensorEvent"/>
	/// </summary>
	[InlineArray(6)]
		...
		IEquatable<SensorData>, IFormattable, ISpanFormattable, IEqualityOperators<SensorData, SensorData, bool>
	{
		...
		/// <summary>
		/// Creates a new <see cref="SensorData"/> with the given sensor data values
		/// </summary>
		...
		/// <param name="data3">The fourth sensor data value</param>
		/// <param name="data4">The fifth sensor data value</param>
		/// <param name="data5">The sixth sensor data value</param>
		[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
		public SensorData(float data0, float data1, float data2, float data3, float data4, float data5)
		{
			this[0] = data0;
			this[1] = data1;
			this[2] = data2;
		...
		public readonly string ToString(string? format, IFormatProvider? formatProvider)
			=> $"[ {this[0].ToString(format, formatProvider)}, {
				...
				this[3].ToString(format, formatProvider)}, {
				this[4].ToString(format, formatProvider)}, {
				this[5].ToString(format, formatProvider)} ]";
		...
		public readonly bool TryFormat(Span<char> destination, out int charsWritten, ReadOnlySpan<char> format = default, IFormatProvider? provider = default)
		{
			...
			return SpanFormat.TryWrite("[ ", ref destination, ref charsWritten)
				...
				&& SpanFormat.TryWrite(", ", ref destination, ref charsWritten)
				&& SpanFormat.TryWrite(this[3], ref destination, ref charsWritten, format, provider)
				&& SpanFormat.TryWrite(", ", ref destination, ref charsWritten)
				&& SpanFormat.TryWrite(this[4], ref destination, ref charsWritten, format, provider)
				&& SpanFormat.TryWrite(", ", ref destination, ref charsWritten)
				&& SpanFormat.TryWrite(this[5], ref destination, ref charsWritten, format, provider)
				&& SpanFormat.TryWrite(" ]", ref destination, ref charsWritten);
		}
