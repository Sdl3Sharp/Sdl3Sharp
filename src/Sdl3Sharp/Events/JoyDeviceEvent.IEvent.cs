...
partial struct Event
{
	...
	partial interface IUnionMembers
	{
		...
		static abstract Event Create(in JoyDeviceEvent value);
		...
		/// </para>
		/// </remarks>
		bool TryGetValue(out JoyDeviceEvent value);
	}
#endif
	...
	static Event IUnionMembers.Create(in JoyDeviceEvent value) => From(in value);

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly bool IUnionMembers.TryGetValue(out JoyDeviceEvent value) => TryGet(out value);
#endif
}

partial struct JoyDeviceEvent : IEvent<JoyDeviceEvent>
{
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	private static bool AcceptsEventType(EventType type) => type is EventType.JoystickAdded or EventType.JoystickRemoved or EventType.JoystickUpdateCompleted;

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	static bool IEvent<JoyDeviceEvent>.TryReadFromEvent(ref readonly Event @event, out JoyDeviceEvent result)
	{
		if (AcceptsEventType(@event.Type))
		{
			result = @event.JDevice;
			return true;
		}
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly void IEvent<JoyDeviceEvent>.WriteToEvent(ref Event @event) => @event.JDevice = this;
}
