...
partial struct Event
{
	...
	partial interface IUnionMembers
	{
		...
		static abstract Event Create(in JoyButtonEvent value);
		...
		/// </para>
		/// </remarks>
		bool TryGetValue(out JoyButtonEvent value);
	}
#endif
	...
	static Event IUnionMembers.Create(in JoyButtonEvent value) => From(in value);

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly bool IUnionMembers.TryGetValue(out JoyButtonEvent value) => TryGet(out value);
#endif
}

partial struct JoyButtonEvent : IEvent<JoyButtonEvent>
{
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	private static bool AcceptsEventType(EventType type) => type is EventType.JoystickButtonDown or EventType.JoystickButtonUp;

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	static bool IEvent<JoyButtonEvent>.TryReadFromEvent(ref readonly Event @event, out JoyButtonEvent result)
	{
		if (AcceptsEventType(@event.Type))
		{
			result = @event.JButton;
			return true;
		}
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly void IEvent<JoyButtonEvent>.WriteToEvent(ref Event @event) => @event.JButton = this;
}
