...
partial struct Event
{
	...
	partial interface IUnionMembers
	{
		...
		static abstract Event Create(in PinchFingerEvent value);
		...
		/// </para>
		/// </remarks>
		bool TryGetValue(out PinchFingerEvent value);
	}
#endif
	...
	static Event IUnionMembers.Create(in PinchFingerEvent value) => From(in value);

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly bool IUnionMembers.TryGetValue(out PinchFingerEvent value) => TryGet(out value);
#endif
}

partial struct PinchFingerEvent : IEvent<PinchFingerEvent>
{
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
#pragma warning disable CS0618 // We can use them here, that's what they're there for
	private static bool AcceptsEventType(EventType type) => type is >= EventType.PinchFirst and <= EventType.PinchLast;
#pragma warning restore CS0618

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	static bool IEvent<PinchFingerEvent>.TryReadFromEvent(ref readonly Event @event, out PinchFingerEvent result)
	{
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly void IEvent<PinchFingerEvent>.WriteToEvent(ref Event @event) => @event.Pinch = this;
}
