...
partial struct Event
{
	...
	partial interface IUnionMembers
	{
		...
		static abstract Event Create(in PenButtonEvent value);
		...
		/// </para>
		/// </remarks>
		bool TryGetValue(out PenButtonEvent value);
	}
#endif
	...
	static Event IUnionMembers.Create(in PenButtonEvent value) => From(in value);

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly bool IUnionMembers.TryGetValue(out PenButtonEvent value) => TryGet(out value);
#endif
}

partial struct PenButtonEvent : IEvent<PenButtonEvent>
{
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	private static bool AcceptsEventType(EventType type) => type is EventType.PenButtonDown or EventType.PenButtonUp;

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	static bool IEvent<PenButtonEvent>.TryReadFromEvent(ref readonly Event @event, out PenButtonEvent result)
	{
		if (AcceptsEventType(@event.Type))
		{
			result = @event.PButton;
			return true;
		}
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly void IEvent<PenButtonEvent>.WriteToEvent(ref Event @event) => @event.PButton = this;
}
