...
partial struct Event
{
	...
	partial interface IUnionMembers
	{
		...
		static abstract Event Create(in GamepadButtonEvent value);
		...
		/// </para>
		/// </remarks>
		bool TryGetValue(out GamepadButtonEvent value);
	}
#endif
	...
	static Event IUnionMembers.Create(in GamepadButtonEvent value) => From(in value);

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly bool IUnionMembers.TryGetValue(out GamepadButtonEvent value) => TryGet(out value);
#endif
}

partial struct GamepadButtonEvent : IEvent<GamepadButtonEvent>
{
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	private static bool AcceptsEventType(EventType type) => type is EventType.GamepadButtonDown or EventType.GamepadButtonUp;

	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	static bool IEvent<GamepadButtonEvent>.TryReadFromEvent(ref readonly Event @event, out GamepadButtonEvent result)
	{
		if (AcceptsEventType(@event.Type))
		{
			result = @event.GButton;
			return true;
		}
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
	readonly void IEvent<GamepadButtonEvent>.WriteToEvent(ref Event @event) => @event.GButton = this;
}
